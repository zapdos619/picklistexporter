...
3. Update credentials and Run:
   METHOD - Environment Variables (recommended for security):
    ...
    for linux/mac
        ...
        export SF_DOMAIN='your_domain (login or test)'

        # Step 2: Run the script (in the SAME terminal)
        python picklist_export.py
        
    for Windows Powershell:
        # Step 1: Set environment variables
        $env:SF_USERNAME='your_username@example.com'
        $env:SF_PASSWORD='your_password'
        $env:SF_SECURITY_TOKEN='your_security_token'
        $env:SF_DOMAIN='your_domain (login or test)'
    ...
    for Windows command Prompt:
        Step 1: Set environment variables
        set SF_USERNAME=your_username@example.com
        set SF_PASSWORD=your_password
        set SF_SECURITY_TOKEN=your_security_token
        set SF_DOMAIN=your_domain (login or test)
