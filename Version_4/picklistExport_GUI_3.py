...
import threading # <-- NEW: For running tasks off the main GUI thread
...
# ===========================================
# HELPER CLASSES (Core logic remains the same)
# ===========================================
...
# ===========================================
# MAIN EXPORT CLASS (Core logic remains the same)
# ===========================================
...
class PicklistExporter:
    ...
    def __init__(self, username: str, password: str, security_token: str, domain: str = 'login', status_callback=None):
        ...
        if self.status_callback: self.status_callback("Initializing Salesforce Connection...")
        ...
        try:
            ...
            if self.status_callback: self.status_callback(f"✅ Connected to: {self.base_url}")
            self._fetch_all_org_objects()
        except Exception as e:
            if self.status_callback: self.status_callback(f"❌ Connection failed: {str(e)}")
            raise
    ...
    def _log_status(self, message: str):
        if self.status_callback: self.status_callback(message, verbose=True) 

    # --- Core export methods (kept intact for functionality) ---
        ...
        for i, obj_name in enumerate(object_names, 1):
            ...
            try:
                ...
                if not result.object_exists:
                    ...
                    self._log_status(f"  ✅ Fields: {result.picklist_fields_count}, Active: {stats['total_active_values']}, Inactive: {result.inactive_values}")
            except Exception as e:
                stats['failed_objects'] += 1
                stats['failed_object_details'].append({'name': obj_name, 'reason': str(e)})
            self._log_status("")
    ...
    def _process_object(self, obj_name: str) -> ProcessingResult:
        ...
        try: getattr(self.sf, obj_name).describe()
        except Exception as e:
            if 'NOT_FOUND' in str(e) or 'INVALID_TYPE' in str(e): result.object_exists = False; return result
            raise
        ...
        for field_api, field_info in picklist_fields.items():
            ...
            for value in values:
                ...
                row = [obj_name, field_info.label, field_api, value.label, value.value, 'Active' if is_active else 'Inactive']
    ...
    def _get_picklist_fields(self, object_name: str) -> Dict[str, FieldInfo]:
        ...
        except Exception as e: self._log_status(f"  ERROR in _get_picklist_fields: {str(e)}")
        return fields_dict
    ...
    def _resolve_entity_definition_id(self, object_name: str) -> Optional[str]:
        ...
        except Exception as e: self._log_status(f"  ERROR resolveEntityDefinitionId: {str(e)}")
        return None
    ...
    def _query_field_definition_tooling(self, object_name: str, field_name: str) -> List[PicklistValueDetail]:
        ...
        except Exception as e: self._log_status(f"      ERROR queryFieldDefinitionTooling: {str(e)}")
        return []
    ...
    def _query_custom_field_tooling(self, entity_def_id: str, field_name: str) -> List[PicklistValueDetail]:
        try:
            ...
            if response.status_code == 200:
                records = response.json().get('records', [])
                if records: return self._parse_value_set(records[0].get('Metadata', {}))
        except Exception as e: self._log_status(f"      ERROR queryCustomFieldTooling: {str(e)}")
        return []
    ...
    def _query_custom_field_tooling_table_enum(self, object_name: str, field_name: str) -> List[PicklistValueDetail]:
        try:
            ...
            if response.status_code == 200:
                records = response.json().get('records', [])
                if records: return self._parse_value_set(records[0].get('Metadata', {}))
        except Exception as e: self._log_status(f"      ERROR queryCustomFieldToolingTableEnum: {str(e)}")
        return []
    ...
    def _query_rest_describe_for_picklist(self, object_name: str, field_name: str) -> List[PicklistValueDetail]:
        ...
        except Exception as e: self._log_status(f"      ERROR queryRestDescribeForPicklist: {str(e)}")
        return []
    ...
    def _parse_value_set(self, metadata: dict) -> List[PicklistValueDetail]:
        ...
        except Exception as e: self._log_status(f"      ERROR parseValueSet: {str(e)}")
        return results
...
class PicklistExportGUI(ctk.CTk):
    def __init__(self):
        ...
        self.title("Salesforce Admin Utility Hub") 
    ...
    # ==================================
    # Screen 2: Admin Utility Hub 
    # ==================================
    
    def _setup_export_frame(self):
        export_frame = self.export_frame
        export_frame.grid_rowconfigure(1, weight=3) # Object Selector gets most space
        export_frame.grid_rowconfigure(3, weight=1) # Status Terminal gets space
        ...
        ctk.CTkLabel(header_frame, text="Salesforce Utility Hub", font=ctk.CTkFont(size=30, weight="bold")).grid(row=0, column=0, sticky="w")
        ...
        # Left Panel: Available Objects (Searchable)
        ...
        ctk.CTkLabel(action_frame, text="Selection Actions", font=ctk.CTkFont(size=14, weight="bold")).pack(pady=5)
        ...
        ctk.CTkLabel(selected_frame, text="Selected for Task", font=ctk.CTkFont(size=18, weight="bold")).grid(row=0, column=0, pady=(5, 5))
        ...
        # --- Row 2: Task Button Panel ---
        task_panel_frame = ctk.CTkFrame(export_frame)
        task_panel_frame.grid(row=2, column=0, pady=(0, 10), padx=20, sticky="ew")
        task_panel_frame.columnconfigure(0, weight=1)
        task_panel_frame.columnconfigure(1, weight=1)
        task_panel_frame.columnconfigure(2, weight=1)
        task_panel_frame.columnconfigure(3, weight=1)
        
        # Current Task Button (Now calls the threading wrapper)
        self.picklist_export_button = ctk.CTkButton(task_panel_frame, text="1. Export Picklist Data", command=self.export_picklist_action, height=50, fg_color="green", font=ctk.CTkFont(size=16, weight="bold"))
        self.picklist_export_button.grid(row=0, column=0, pady=10, padx=10, sticky="ew")

        # Placeholder for Future Tasks
        ctk.CTkButton(task_panel_frame, text="2. Dependency Analysis (Future)", state="disabled", height=50).grid(row=0, column=1, pady=10, padx=10, sticky="ew")
        ctk.CTkButton(task_panel_frame, text="3. Metadata Exporter (Future)", state="disabled", height=50).grid(row=0, column=2, pady=10, padx=10, sticky="ew")
        ctk.CTkButton(task_panel_frame, text="4. Formula Extractor (Future)", state="disabled", height=50).grid(row=0, column=3, pady=10, padx=10, sticky="ew")


        # --- Row 3: Status Terminal (Bigger) ---
        self.status_textbox = ctk.CTkTextbox(export_frame, height=200) # Increased Height
        self.status_textbox.grid(row=3, column=0, padx=20, pady=(10, 20), sticky="nsew")
        self.status_textbox.insert("end", "Status: Ready to select objects and choose a task.")
    ...
    # --- Object List Management Methods (remain the same) ---
    ...
    def add_selected_to_export(self):
        ...
        if added_count > 0:
            ...
            self.update_status(f"Added {added_count} object(s) to selection.")
    ...
    def remove_selected_from_export(self):
        ...
        if not selected_indices:
            messagebox.showwarning("Selection", "Please select one or more objects from the 'Selected for Task' list to remove.")
            return
        ...
        if removed_objects:
            self.populate_selected_objects()
            self.filter_available_objects(None)
            self.update_status(f"Removed {len(removed_objects)} object(s) from selection.")
    ...
    def export_picklist_action(self):
        """Starts the Picklist Export Task in a separate thread to keep the UI responsive."""
        ...
        if not selected_objects_list:
            messagebox.showwarning("Warning", "The 'Selected for Task' list is empty. Please add objects.")
            return
        ...
        # 1. Disable Button and Update Status on MAIN THREAD
        self.picklist_export_button.configure(state="disabled", text="Exporting... DO NOT CLOSE")
        self.update_status(f"Starting Picklist Export for {len(selected_objects_list)} objects to {output_file_path}...")
    ...
    def _run_export_task(self, selected_objects_list: List[str], output_file_path: str):
        """The actual long-running export logic executed in a separate thread."""
        start_time = time.time()
        output_path = None
        stats = None
        
        try:
            # This method contains the time-consuming API calls and file writing.
            ...
            # UI updates (status, message box) happen after the task is done
            ...
            self.picklist_export_button.configure(state="normal", text="1. Export Picklist Data")
